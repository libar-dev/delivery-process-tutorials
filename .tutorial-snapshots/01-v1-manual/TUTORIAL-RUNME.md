...
Start from a clean state. This removes any previously created tutorial files so you can follow along from scratch.

```bash {"excludeFromRunAll":"true","name":"reset-workspace"}
rm -rf src/sample-sources/* src/specs/* src/stubs/* docs-generated/*
...
This project comes pre-configured with `package.json`, `tsconfig.json`, and dependencies installed. Let's verify:

```bash {"closeTerminalOnSuccess":"false","name":"verify-project"}
echo "=== package.json ==="
...
echo "src/sample-sources/ - TypeScript implementation files"
echo "src/specs/          - Gherkin feature specs"
echo "src/stubs/          - Design stub files"
ls -d src/sample-sources src/specs src/stubs 2>/dev/null || echo "Creating folders..."
mkdir -p src/sample-sources src/specs src/stubs
```

### 1.2 Install dependencies (if needed)

Skip this if dependencies are already installed:
